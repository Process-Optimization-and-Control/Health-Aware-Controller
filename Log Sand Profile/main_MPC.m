...
sandArray = sandproductionrate(0.01,simLength,'log',0.02);
...
nmpcConfig.umax = 2;
nmpcConfig.umin = 0.4;
nmpcConfig.dumax = 0.01; % 0.01
nmpcConfig.x_threshold = 0.3;  % 0.8 | 1 | 2
...
for tt = 0:simLength
    ...
    if tt ~=0
        ...
        % perfect information
        [uk,s,erosionHat,inputSeq,OF,solFlag] = SolvingNMPC(solverNMPC,xk,zk,uk,sandArray(tt),par.GOR,par.PI,par.T,nmpcConfig);

        % with diagnostics
        %  [uk,s,erosionHat,inputSeq,OF,solFlag] = SolvingNMPC(solverNMPC,xHat,zk,uk,sandArray(tt + 1),par.GOR,par.PI,par.T,nmpcConfig);
...
save(name,'simLength','tBreak','xPlant','zPlant','yPlant','totalProductionPlant','slackControl','inputControl','flagControl','erosionPrediction','CPUtimeControl','ofControl','erosionHatDiag','inputPrediction')
